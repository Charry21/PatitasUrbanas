...
**Nota de lectura para las columnas "Archivo / módulo real", "Clase, símbolo o configuración verificable" y "Relación verificada" en filas Planificado:** todo valor marcado con el prefijo `(propuesto, no existe aún)` o `(propuesta, no existe aún)` es una **ruta de implementación sugerida para el roadmap**, no evidencia de código. No debe leerse, citarse ni presentarse en la defensa como si ya existiera en el repositorio — cumple la misma función que una nota en el backlog: decirle al equipo dónde deberá aterrizar ese componente cuando se implemente. La columna "Estado" es la que determina la verdad de auditoría; estos rellenos no la cambian.
...
| C4-03 | Nivel 1 | Ciudadano / Adoptante (persona) | Adoptar mascotas vía la plataforma | (propuesto, no existe aún) carpeta `mobile/` (app Android) y/o `web/` (cliente web) | (propuesto, no existe aún) pantallas de registro, catálogo y solicitud de adopción | (propuesta, no existe aún) Ciudadano → Front-end Móvil / Front-end Web (interacción de usuario) | Planificado (no implementado) | Sin canal de interacción implementado (no hay frontend). Declarado en [`dossier/01-contexto-sistema.md`](../dossier/01-contexto-sistema.md) como usuario objetivo futuro. |
| C4-04 | Nivel 1 | Administrador de Refugio (persona) | Gestionar publicaciones de mascotas | (propuesto, no existe aún) carpeta `web/` (panel de administración de fundaciones/veterinarias) | (propuesto, no existe aún) pantalla de gestión de perfiles de mascotas y estados de adopción | (propuesta, no existe aún) Administrador de Refugio → Front-end Web (interacción de usuario) | Planificado (no implementado) | Misma razón que C4-03. |
| C4-05 | Nivel 1 | Servicio de Mapas y Geolocalización (sistema externo) | Renderizar mapas y ubicar georreferencias | (propuesto, no existe aún) integración SDK de mapas (ej. Google Maps SDK o Mapbox) en el cliente móvil | (propuesto, no existe aún) módulo `MapaService`/`GeoClient` en el cliente | (propuesta, no existe aún) Front-end Móvil → Servicio de Mapas (llamada API externa) | Planificado (no implementado) | Sin dependencia, SDK o llamada saliente en el código todavía. |
...
| C4-08 | Nivel 2 | Front-end Web | Interfaz web de usuario final | (propuesto, no existe aún) carpeta `web/` en la raíz del repositorio, desplegado en Vercel | (propuesto, no existe aún) proyecto Next.js/React con páginas de catálogo, login y gestión | (propuesta, no existe aún) Front-end Web → API Backend (HTTPS/REST) | Planificado (no implementado) | Restricción de despliegue en Vercel ya declarada en el dossier, sin proyecto todavía. |
| C4-09 | Nivel 2 | Front-end Móvil (Kotlin / Jetpack Compose) | Interfaz móvil nativa | (propuesto, no existe aún) carpeta `mobile/` en la raíz del repositorio | (propuesto, no existe aún) `MainActivity` y pantallas Jetpack Compose (catálogo, mapa, perfil) | (propuesta, no existe aún) Front-end Móvil → API Backend (HTTPS/REST) | Planificado (no implementado) | Restricción técnica ya declarada en el dossier, sin módulo Android todavía. |
| C4-10 | Nivel 2 | Caché / Almacenamiento no estructurado (Firestore/MongoDB) | Persistencia de foros y geolocalización efímera | N/A — eliminado, sin ruta futura aplicable | N/A — eliminado, sin ruta futura aplicable | N/A — eliminado, sin ruta futura aplicable | Eliminado (decisión de arquitectura) | Descartado por decisión de equipo ([`dossier/02-stakeholders-drivers.md`](../dossier/02-stakeholders-drivers.md), riesgo R-03); no es trabajo pendiente, así que no aplica una ruta propuesta. |
...
| C4-13 | Nivel 3 | Módulo de Autenticación y Autorización | Validar credenciales y controlar acceso por roles | (propuesto, no existe aún) paquete `app/src/main/java/com/patitasurbanas/api/security/` + dependencia `spring-boot-starter-security` en `pom.xml` | (propuesto, no existe aún) `SecurityConfig`, filtro JWT, `UsuarioController` | (propuesta, no existe aún) Módulo de Autenticación → intercepta todos los controladores REST (filtro de seguridad) | Planificado (no implementado) | Sin dependencia `spring-security` ni clases de seguridad en [`app/src`](../app/src) todavía. Corresponde al driver QA-01 del dossier. |
| C4-14 | Nivel 3 | Módulo de Gestión de Adopciones y Mascotas | Registro y consulta de perfiles de animales | (propuesto, no existe aún) paquetes `model/`, `repository/`, `service/` para `SolicitudAdopcion`/`EtapaAdopcion` en `app/src/main/java/com/patitasurbanas/api/` | (propuesto, no existe aún) entidades JPA, `AdopcionService` transaccional, `AdopcionController` | (propuesta, no existe aún) Módulo de Adopciones → Base de Datos Transaccional (JDBC/JPA) | Planificado (no implementado) | Sin paquetes `controller/service/repository/model` todavía **en el commit auditado**. Corresponde al driver QA-02 del dossier; hay una implementación en curso en entorno local, aún sin commit ni test verificado — no se cuenta como evidencia hasta que se confirme con `mvn test` en verde y se suba al repositorio. |
| C4-15 | Nivel 3 | Módulo de Reportes de Animales Perdidos | Procesar coordenadas y alertas de mascotas extraviadas | (propuesto, no existe aún) paquete `app/src/main/java/com/patitasurbanas/api/reporte/` | (propuesto, no existe aún) `ReporteController`, `ReporteService`, entidad `ReporteMascotaPerdida` | (propuesta, no existe aún) Ciudadano (vía Front-end) → Módulo de Reportes → Base de Datos Transaccional | Planificado (no implementado) | Sin código correspondiente en el repositorio todavía. |
| C4-16 | Nivel 3 | Módulo de Foros Comunitarios | Publicaciones e hilos de discusión | (propuesto, no existe aún) paquete `app/src/main/java/com/patitasurbanas/api/foro/` | (propuesto, no existe aún) `ForoController`, `ForoService`, entidades `Hilo`/`Comentario` | (propuesta, no existe aún) Front-end → Módulo de Foros → Base de Datos Transaccional | Planificado (no implementado) | Sin código correspondiente todavía; su almacenamiento (C4-10) fue descartado, así que su implementación futura deberá apoyarse en PostgreSQL, no en Firestore/MongoDB. |
